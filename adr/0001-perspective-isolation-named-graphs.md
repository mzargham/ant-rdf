...
4. **Implementation deferred.** The change is tool-level (loader graph assignment, every compiler's scope, the verify target of Decision 2) and changes the CLI's structural contract, so it lands as a coordinated migration, not piecemeal. Until then we **proceed single-graph**.
