...
A named analysis scope on a fieldsite. A perspective is the relation between a perspective-haver (an analyst or team) and what is being seen — so perspectives belong inside cases (per R9a). In v1 a first-class RDF subject; in v2 the same IRI becomes the URI of a named graph (no migration). See C6.
