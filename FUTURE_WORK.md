...
**The frame↔network join is the clearest thing the named-graph lift buys.** `network_for_perspective` and `perspective_for_network` (`compilers/_common.py`) pair a frame with its network by **tail-slug match** (`perspectives/<x>` ↔ `network/<x>`), falling back to the lone network / lone grounded perspective when a case has exactly one. [ADR-0001](adr/0001-perspective-isolation-named-graphs.md) already names this as convention-not-structure and already decides the fix — *"the file-scoping and slug-matching conventions retire"* under one named graph per perspective — so this is not an open design question, only an unscheduled one. Its Consequences section is explicit that the convention **stays until that lift lands**, because the conventional joins depend on it.

What the convention costs in the meantime, now observed rather than predicted: it forces the network's identifier to carry the *perspective's* slug, so a frame phrased as a reading ("MIRA as organised effort") yields a network IRI that disagrees with the network's own label ("the MIRA workshop") — reported from first contemporary use of the catechism (PR #7 review, 2026-08). And a multi-frame case whose slugs drift loses its frame pairing silently rather than failing loudly.

**Resist the tempting shortcut.** An explicit `ant:networkUnderPerspective` predicate looks like a cheap fix and is not one: it introduces a *second* mechanism for frame membership alongside the graph-membership answer R8a already commits to, so every compiler would have to reconcile the two, and a case could assert an edge contradicting the graph it sits in. A new term also needs `dcterms:source` to a founding text (R9) — and there is no founding text for a join key, because this is a storage artifact, not a material-semiotic claim. Under named graphs the question answers itself: **the network declared in a perspective's graph is that frame's network**, no predicate and no slug required.

*What (per R8a):* the lift is `publicID` on parse, not a data migration — `graph.py` already uses a quad-capable `Dataset`, records already live under `instances/cases/<case>/perspectives/<slug>/`, and an `ant:Perspective` IRI is already *the name of the graph* that directory routes into. Serializing as TriG makes the partition visible on disk, and the two resolver functions collapse into a graph-membership lookup. *Then:* per ADR-0001 Decision 2, SHACL runs per graph for structural correctness and over the union for cross-frame checks; every `queries/*.rq` matching the default graph needs a `GRAPH ?frame` clause (each already carries that forward-compat note). *Until then:* the convention holds, and the `ant-mgmt` catechism now states and confirms the slug when it mints it — with the reason — rather than choosing silently.
