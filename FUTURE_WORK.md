...
`ant-rdf` v0.1.0-draft ships a working v1 that we expect to **validate, calibrate, and extend through use**. This document records what was deliberately left out of v1, how we expect those omissions to be implemented as the toolkit matures, and the three-tier validation regime that should govern whether and how this machine-mediated method is permitted to produce novel ethnographic claims.
...
## 1. What v1 deliberately left out of scope
...
### 1.1 Multiplicity proper — named graphs over a quadstore
...
- Flip `graph.py`'s loader to use per-perspective `publicID` arguments; each `ant:Perspective` IRI becomes a named graph URI. No data migration required (per R8) — the file-system layout already names the v2 graphs.
...
### 1.2 Rule-based tagging engine for analytical enrichment
...
**Why deferred.** The data shape and the philosophical positioning (per R6 in ADR-0000) needed to land before the computation engine. v1 demonstrates that manually-asserted Characterizations work end-to-end; v2 adds the second path. Both produce identical RDF; provenance distinguishes their origin.
...
**Out of v1 scope.** Translations, moments, characterizations, durability assertions — none carry datetime properties in v1. The ontology is atemporal by design choice.

**Why deferred.** Temporality is important for tracing controversy dynamics, network durability decay, and the precariousness Callon and Law emphasize. But getting the structural mechanics right first (per the user's directive) avoids the kind of half-modeled temporal scheme that becomes harder to fix than to add.

**Implementation path (v2).**
- Translation moments already subclass `prov:Activity` (under the optional alignment), so `prov:startedAtTime` / `prov:endedAtTime` properties drop in with zero schema migration.
- Add Tier-2 warning that translations *should* carry an interval (waivable for ethnographic accounts where dates aren't separable from the field's own sense of time).
- Add a `TranslationTimeline` compiler — render a translation's moments as a horizontal timeline rather than a four-section narrative.
- Reckon honestly with cases where the source material doesn't have datable moments (Callon's scallops paper uses seasons, not dates; Latour's hotel-keys is a generic example without specific times).
...
**Out of v1 scope.** v1's `ant:enrols`, `ant:translates`, `ant:passesThrough`, `ant:speaksFor` are binary properties. The PROV qualified-influence pattern (where a relation gets reified as a node carrying strength, time, contestation status, justification, etc.) is the natural v1.1 extension.
...
**Implementation path (v1.1).**
- Add `ant:EnrolmentRelation`, `ant:TranslationRelation`, `ant:RolePlaying` as reified n-ary nodes with `ant:strength`, `ant:isContested`, `ant:contestedBy`, plus the v2 temporal properties.
...
**Implementation path (v0.1.0-final).**
...
- *The Body Multiple* atherosclerosis (Mol 2002) — multiplicity, demands v2 named graphs
...
- de Laet & Mol's Zimbabwe bush pump (2000) — fluid technology that should *break* `ant:ImmutableMobile` in interesting ways and force the v2 `ant:FluidObject` lift.
...
- **Each new feature ships with a training case that uses it.** If the v1.1 Akrich case requires reified `EnrolmentRelation`, the PR that adds the class also adds the case-study record that motivates the class.
...
- **Comparison-study findings change the ontology.** If a paired-analysis study shows the toolkit's Mediator/Intermediary characterization systematically obscures something the unaided analyst captures, that's data the v2 ontology must respond to — not a deficiency of the unaided analyst.
...
- Confirm the priorities of §1 above (which v1.1 / v2 features land first) against ethnographer-team workload.
