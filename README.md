...
- **The docs-as-code-with-semantic-web pattern is replicating.** It is one of several repositories Michael Zargham has shaped around the same scaffolding (canonical RDF + deterministic compilers + LLM-callable CLI + reviewable briefs + an authoring contract for humans and LLMs). The first crystallisation is [BlockScience/trl-knowledgebase](https://github.com/BlockScience/trl-knowledgebase) (Technology Readiness Level documentation); [`RIME-product-docs`](https://github.com/dynamicalsystems-group/RIME-product-docs) is the immediate template `ant-rdf` was scaffolded against. The pattern is taking on the character of a small *pattern language* because its ergonomics suit the kind of accountability LLM-mediated work — widely adopted and frequently mindlessly so — has lacked. Lineage details and citations are in the [abstract](abstract.md).
